 ...
 * @param cwd
 * @param pkg
 * @returns eslint-config-cyril/index
 * @returns eslint-config-cyril/react
 * @returns eslint-config-cyril/typescript/index
 * @returns eslint-config-cyril/typescript/react
 */
export function getESLintConfigType(cwd: string, pkg: PKG): string {
  ...
  return (
    'eslint-config-cyril/' + `${language}/${dsl}`.replace(/\/$/, '/index').replace(/^\//, '')
  );
}
